<?php
 ...
 //DADOS DA EQUIPE --------------------------------
  $nomee = $_POST['nomee'];
  $cnpje = $_POST['cnpje'];
  $cnpjfiltered = preg_replace("/[^0-9]/", "", "$cnpje" );
 ...
 // Especializaçao:--------------------------------
   ...
   $sqlsearch = "SELECT * FROM `equipe` WHERE cnpj_equipe = '$cnpjfiltered'";
   $resultsearch = mysqli_query($conn, $sqlsearch);
   if($row = $resultsearch->fetch_assoc()){ echo "<br> tem equipe com esse cnpj"; }
   else{

     $sqlsearchtemp = "SELECT * FROM `equipetemp` WHERE cnpj_equipe = '$cnpjfiltered'";
     $resultsearchtemp = mysqli_query($conn, $sqlsearchtemp);
     if($row = $resultsearchtemp->fetch_assoc()){ echo "<br> tem equipe temporaria com esse cnpj"; }
     else{
 ?>
   ...
   <main>
     <form class="box cadastro" enctype='multipart/form-data' action="../../app/cadequipetemp.app.php" method="post">
       ...
       for ($i=1; $i<=$integrantese;$i++) {
         ...
         <input type='file' name='curriculoi$i' required/></p> <br>
